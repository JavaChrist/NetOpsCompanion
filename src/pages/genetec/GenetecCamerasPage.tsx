...
import { Camera, BookOpen, Stethoscope, ArrowRight } from 'lucide-react';
import { procedures, diagnostics } from '../../data/genetec';
import { GenetecBreadcrumb } from '../../components/genetec/GenetecBreadcrumb';

const videoProcedures = procedures.filter(p => p.domain === 'video');
const videoDiagnostics = diagnostics.filter(d => d.domain === 'video');
...
export function GenetecCamerasPage() {
  return (
    <div className="space-y-8">
      <div>
        <GenetecBreadcrumb crumbs={[{ label: 'Vidéo / Caméras' }]} />
        <h1 className="text-2xl font-bold text-white mb-1">Vidéo / Caméras</h1>
        <p className="text-slate-400 text-sm">
          Configuration guidée et diagnostic des caméras IP dans Genetec Security Center.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Link to="/genetec/logs" className="group flex items-center gap-3 rounded-xl border border-surface-600 bg-surface-800 px-4 py-3 hover:border-accent/40 hover:bg-surface-700 transition-all">
          ...
          <span className="text-sm text-slate-300 group-hover:text-white">Référence logs vidéo</span>
          <ArrowRight className="w-4 h-4 text-slate-600 group-hover:text-accent ml-auto" />
        </Link>
      </div>
    </div>
